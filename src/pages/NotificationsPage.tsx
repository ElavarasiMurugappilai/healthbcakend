...
const NotificationsPage: React.FC<NotificationsPageProps> = ({ searchValue }) => {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    ...
    { id: 7, type: "Challenge", title: "Weekly Goal Achieved", desc: "Congratulations! You've completed your weekly fitness goal.", read: false, time: "1 week ago" },
    { id: 8, type: "System", title: "New Feature Available", desc: "Check out our new medication tracking feature!", read: true, time: "1 week ago" }
  ]);
  ...
  const handleToggleRead = (id: number) => {
    setNotifications(prev => {
      ...
      if (updated.every(n => n.read)) {
        // Add a new notification after a delay
        setTimeout(() => {
  ...
  return (
    <div className="p-4 w-full max-w-6xl mx-auto space-y-6 overflow-x-hidden overflow-y-hidden min-h-screen">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button onClick={handleMarkAllRead} className="bg-primary text-primary-foreground hover:bg-primary/90">
          Mark all as read
        </Button>
      </div>
      ...
      {/* Notification List */}
      <Card className="bg-gray-100 dark:bg-gray-900 border-none dark:border-zinc-800 shadow p-4 w-full text-black dark:text-white">
                ...
                {filtered.map(n => {
                  ...
                  >
                    <div className="flex-1 min-w-0 flex flex-col justify-center space-y-2">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge 
                          ...
                          className={`w-2 h-2 rounded-full flex-shrink-0 p-0 ${n.read ? "bg-gray-400" : "bg-orange-500"}`}
                        />
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <Badge variant="outline" className="text-xs text-muted-foreground whitespace-nowrap hidden sm:block">
                        {n.time}
                      </Badge>
                      <Button
                        variant="ghost"
